// @aval/mcp — src/tools/aval_report.ts — docs/06-mcp-skills.md §2.12b
//
// aval_report(target, evidenceURI, weightPoints) -> { reportId, bond,
...
// AVAL_OPERATOR_ADDRESS (see ToolContext / mcp/README.md) — docs/06's
// signature has no explicit `reporter` parameter, so the server's own
// identity is used, matching how aval-mcp is meant to run: "at its
...
// signer this scaffold doesn't wire up; see aval_request_score.ts for the
...
export const tool: ToolDefinition<typeof inputSchema> = {
  ...
  async handler(args, ctx) {
    ...
    if (isPlatformReporter) {
      ...
      // vouch computation aval_platform.ts does; reusing that here would
      ...
      if (!platform!.platform.active) {
        return errorResult(new AvalToolError("InsufficientTier", "operator platform is not active"));
      }
    } else {
      const reporterResult = scored.scores.get(operator);
      if (!reporterResult || reporterResult.tier < 1) {
        return errorResult(
          new AvalToolError("InsufficientTier", `operator tier ${reporterResult?.tier ?? 0} < required 1 (human) or P1 (platform)`),
        );
      }
    }
    ...
    if (isPlatformReporter) {
      ...
      if (!hasRequest) {
        return errorResult(new AvalToolError("NoConnection", "platform has no prior ScoreRequest against this target"));
      }
    } else {
      ...
      if (!path || path.length > 2) {
        return errorResult(new AvalToolError("NoConnection", "no <=2-hop vouch path between operator and target"));
      }
    }
    ...
    if (openByOperator.length >= concurrentLimit) {
      return errorResult(
        new AvalToolError("ReportLimitReached", `operator already has ${openByOperator.length}/${concurrentLimit} open reports`),
      );
    }
    ...
    if (recentFromOperator) {
      return errorResult(new AvalToolError("PairCooldown", "operator reported this target within the last 180 days"));
    }
    ...
    // mcp/README.md's scope note, same as gateway's `aval.bonded`.
    ...
    if (isVoided(reportsAgainstOperator, now)) {
      return errorResult(new AvalToolError("ReporterVoided", "operator is under an upheld report; all of its reports are voided"));
    }
    ...
    return jsonResult({
      ...
      state: "PENDING",
      youAreRisking:
        `${requiredBond} AVAL. If the target's vouchers rebut and arbitration rules MALICIOUS, ` +

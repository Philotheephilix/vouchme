// @vouchme/gateway — src/resolve.test.ts
//
...
test("encodeDnsName / decodeDnsName round-trip", () => {
  const labels = ["carol", "alice", "vouchme", "eth"];
...
test("parseName computes depth from label count (docs/04-ens.md §1)", () => {
  const alice = parseName(encodeDnsName(["alice", "vouchme", "eth"]));
  assert.equal(alice?.depth, 1);

  const carol = parseName(encodeDnsName(["carol", "alice", "vouchme", "eth"]));
  assert.equal(carol?.depth, 2);
  assert.deepEqual(carol?.pathLabelsRootFirst, ["alice", "carol"]);

  const erin = parseName(encodeDnsName(["erin", "carol", "alice", "vouchme", "eth"]));
  assert.equal(erin?.depth, 3);
});

test("names beyond max_depth do not resolve (docs/04-ens.md §5.1)", () => {
  const tooDeep = parseName(encodeDnsName(["w", "x", "y", "z", "vouchme", "eth"]));
  assert.equal(tooDeep, null);
});

test("names outside vouchme.eth do not resolve", () => {

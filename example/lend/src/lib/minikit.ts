 ...
 *
 * Calling `walletAuth()` on the imported MiniKit without installing first fails inside World App
 * with "wallet-auth is unavailable: World App version does not support this command" — not because
 * the version is old, but because `install()` never ran, so MiniKit never completed the host
 * handshake that tells it which commands exist. Every command then reports unsupported.
 *
 * Two further traps, both already learned the hard way in the VouchMe app and in Fiar, and mirrored
 * here so an integrator copying this file gets the working version rather than the obvious one:
 *
 ...
 *
 * Both surface as the identical string — `wallet-auth is unavailable: World App version does not
 * support this command` — and they need opposite responses: one is our bug, the other is the user's
 * app being out of date. minikit-js decides availability from the host's declared support map,
 * which only exists after `install()` completes the handshake, so before install every command
 * reports unsupported and the message blames the version.
 */
 ...
 *
 *  Call this on mount AND again before every command: a page reload resets this module's state,
 *  so the flag above is a cache, never the source of truth. */

/**
 * Server-side sign-in for Fiar.
 *
 * Reading a VouchMe score needs no authentication — scores are public, and the catalogue quotes
 * anyone's price to anyone. Sign-in exists for exactly one reason: the moment Fiar takes a deposit,
 * it must know the person holding the phone controls the address whose reputation earned the
 * discount. Otherwise anyone types an anchor's address and borrows a camera at the 15% floor.
 *
 ...
 *
 * Mirrors VouchMe's own `app/src/lib/authSession.ts` deliberately: an integrator copying this file
 * should end up with the same guarantees the protocol's own app has, not a weaker imitation.
 */
...
export const SESSION_COOKIE = "fiar_session";

const SESSION_TTL_SECONDS = 60 * 60 * 24 * 7; // a week; a lending library is not a bank
const NONCE_TTL_SECONDS = 5 * 60;

export class AuthConfigError extends Error {}

function getSecret(): string {
  const key = process.env.FIAR_SESSION_SECRET;
  if (!key || key.length < 16) {
    throw new AuthConfigError(
      "FIAR_SESSION_SECRET is not set (or is shorter than 16 characters). Fiar cannot issue or " +
 ...
 *
 * Pinned to `globalThis` for the same reason as the pending-payments map: route handlers do not
 * share module instances, so a module-level Map would let the same nonce be redeemed once per
 * instance. Single-use is a security property, and it must not depend on which handler happens to
 * be loaded.
 *
 * A restart makes old nonces unredeemable, which is the safe direction. A multi-instance
 * deployment needs shared storage here.
 */
const nonceStore = globalThis as typeof globalThis & { __fiarConsumedNonces?: Map<string, number> };
const consumedNonces: Map<string, number> = (nonceStore.__fiarConsumedNonces ??= new Map());
...
/** Its own tiny client rather than sharing one with the data layer: signature checks must keep
...
export function signSession(address: Address): { value: string; maxAge: number } {
  const payload: SessionPayload = { address, exp: Math.floor(Date.now() / 1000) + SESSION_TTL_SECONDS };
...
/**
 * A tamper-evident, expiring token the client may hold but cannot edit.
 *
 * Used for the borrow authorization: the price is decided on the server, sealed here, and handed
 * to the client. When the payment step arrives it presents the seal, and Fiar charges what the
 * seal says rather than what the request says. Without this, the deposit amount is a number the
 * client posts, and a client-posted price is not a price.
 *
 * `purpose` is part of the key, so a token minted for one use cannot be replayed as another.
 */
export function signBlob(purpose: string, payload: Record<string, unknown>, ttlSeconds: number): string {
...
export function verifyBlob<T extends Record<string, unknown>>(purpose: string, token: string): (T & { exp: number }) | null {
  ...
  try {
    payload = JSON.parse(Buffer.from(body, "base64url").toString("utf8")) as T & { exp: number };
  } catch {
    return null;
  }

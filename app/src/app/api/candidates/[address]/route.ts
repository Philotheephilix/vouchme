...
export async function GET(_req: Request, { params }: { params: Promise<{ address: string }> }): Promise<Response> {
  ...
  const candidates = data.getCandidates(decoded.value);
  if (!candidates) return fail(404, "identity_not_found", `No candidate list for "${address}".`, data.meta);
  return ok(candidates, data.meta);
}

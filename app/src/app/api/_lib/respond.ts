...
/**
 * `decodeURIComponent` throws a `URIError` on malformed percent-encoding (e.g. a bare "%"). Every
 * dynamic GET route decodes a path segment as its first move, so an uncaught throw there escapes
 * the route handler entirely and Next.js turns it into a generic 500 instead of the standard error
 * envelope. Centralized here — once — instead of a try/catch around `decodeURIComponent`
 * copy-pasted into every one of the five route files.
 */
...
export function decodeParam(raw: string): DecodeParamResult {
  try {
    ...
    // every block explorer and most copy-paste sources produce — would match nothing. Casing is a
    // rendering detail of the same 20 bytes, so it is normalised at the boundary rather than by
    // loosening each comparison inside.
...
/**
 * A POST body of JSON `null` (or an array/primitive) parses successfully via `req.json()` — only
 * a genuinely malformed JSON *document* throws — so the

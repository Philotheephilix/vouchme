...
/** Independent readback: what `<label>.vouchme.eth` actually resolves to right now, read live from
...
export async function GET(_req: Request, { params }: { params: Promise<{ label: string }> }): Promise<Response> {
  ...
  try {
    ...
    return ok({ label: decoded.value, handle: `${decoded.value}.vouchme.eth`, address }, FALLBACK_META);
  } catch (err) {

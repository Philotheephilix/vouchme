...
human — weakly. VouchMe closes the gap: humans who are already trusted vouch for other humans, and your
score is a function of *who* vouched for you, weighted by how trusted *they* are.

The name is the whole mechanism: you vouch, and it is *you* who is on the line. In Portuguese
commercial law the word for that is an **aval** — a personal guarantee written on someone else's
promissory note, signed in your own name. That is exactly what a vouch is here, and it is what this
project was called before it was called VouchMe.
...
3. **Money makes claims cost something; it never makes them true.** `VOUCHME` is a bond for filing and
   contesting reports. 1,000,000 VOUCHME and no vouches is still score 10.
...
A vouch **is** an ENS subname: `carol.alice.vouchme.eth` means Alice vouched for Carol. Depth in the
...
| Path | What |
|---|---|
| `contracts/` | Foundry — `VouchMeRegistry`, `ReportRegistry`, `PlatformRegistry`, `CredibilityVault`, `PresenceDrip` (World Chain, chainId 480) |
...
| `mcp/` | `vouchme-mcp` — MCP server exposing trust queries to agents |
